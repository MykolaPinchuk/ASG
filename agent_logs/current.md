...
- 2026-01-26 20:40:23 PST — Improved `openai_compat` robustness for reasoning-style providers: can parse JSON from `message.reasoning*` fields when present, and retries on `finish_reason=length` with a more conservative max-token bump.
- 2026-01-26 20:44:16 PST — Fixed `openai_compat` retry timeouts exceeding the configured budget: retries now share a single wall-clock deadline so the agent server responds within `--timeout-ms` (prevents agent HTTP timeouts due to multi-attempt calls).
